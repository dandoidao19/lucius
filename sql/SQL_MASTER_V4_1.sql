-- ==============================================================================
-- SCRIPT SQL NUCLEAR LUCIUS V4.1 - CONTADOR SEQUENCIAL DEFINITIVO
...
BEGIN
    ...
    -- 1. CRIAR SEQUÊNCIA PARA NÚMEROS DE TRANSAÇÃO
    ...
    -- 2. SINCRONIZAR SEQUÊNCIA COM O MAIOR NÚMERO ATUAL (Ignorando números "aleatórios" > 9.000.000)
    PERFORM setval('public.seq_numero_transacao_loja',
        COALESCE((
            SELECT MAX(max_num) FROM (
                SELECT MAX(numero_transacao) as max_num FROM public.compras WHERE numero_transacao < 9000000
                UNION ALL
                SELECT MAX(numero_transacao) as max_num FROM public.vendas WHERE numero_transacao < 9000000
                UNION ALL
                SELECT MAX(numero_transacao) as max_num FROM public.transacoes_loja WHERE numero_transacao < 9000000
                UNION ALL
                SELECT MAX(numero_transacao) as max_num FROM public.transacoes_condicionais WHERE numero_transacao < 9000000
            ) t
        ), 0) + 1, false
    );

    -- 3. COLUNAS DE USUÁRIO (Garantia de v3.5+)
    ...
    -- 4. COLUNAS DE VINCULAÇÃO (Garantia de v3.9)
...
-- 6. FUNÇÃO SEQUENCIAL DE TRANSAÇÃO (UTILIZANDO SEQUÊNCIA REAL)
-- Isso garante que chamadas simultâneas não gerem o mesmo número.

// src/components/CaixaCasaDetalhado.tsx
'use client'

import { useCaixaUniversal } from '@/hooks/useCaixaUniversal'
import { formatarMoeda, formatarMoedaCompacta } from '@/lib/utils'

interface CaixaCasaDetalhadoProps {
  titulo?: string
}

export default function CaixaCasaDetalhado({ titulo }: CaixaCasaDetalhadoProps) {
  const {
    caixaRealCasa,
    caixaPrevistoGeral,
    entradasHoje,
  ...
  const renderBotoesModo = () => {
    return (
      <div style={botoesContainerStyle}>
        {filtro !== 'mes' && (
          <button onClick={handleMudarParaMes} disabled={carregando} className="px-1.5 py-0.5 bg-blue-500 text-white rounded text-xs font-medium">
            Mês
          </button>
        )}
        {filtro !== '30dias' && (
          <button onClick={handleVoltar30Dias} disabled={carregando} className="px-1.5 py-0.5 bg-gray-500 text-white rounded text-xs font-medium">
            30 Dias
          </button>
        )}
        {filtro !== 'tudo' && (
          <button onClick={handleMostrarHistorico} disabled={carregando} className="px-1.5 py-0.5 bg-green-500 text-white rounded text-xs font-medium">
            Tudo
          </button>
        )}
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-1 space-y-1" style={{ minWidth: 0 }}>
      <h2 className="font-semibold text-gray-800" style={{ fontSize: '12px' }}>{titulo || 'Caixa Casa'}</h2>

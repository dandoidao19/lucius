        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border">
            ...
            <div>
              ...
              <p className="text-lg font-black text-blue-700">R$ {dadosResumo.total.toFixed(2)}</p>
            </div>
          </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              <table className="w-full text-xs text-left">
                <thead className="bg-gray-100 border-b">
                  <tr>
                    <th className="px-4 py-2 font-bold text-gray-600">Descrição</th>
                    <th className="px-4 py-2 font-bold text-gray-600">Categoria</th>
                    <th className="px-4 py-2 font-bold text-gray-600 text-center">Qtd</th>
                    <th className="px-4 py-2 font-bold text-gray-600 text-right">Unitário</th>
                    <th className="px-4 py-2 font-bold text-gray-600 text-right">Subtotal</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {loading ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">Buscando itens...</td></tr>
                  ) : itens.length === 0 ? (
                    ...
                    itens.map(item => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-right font-bold">R$ {item.subtotal.toFixed(2)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
                {!loading && itens.length > 0 && (
                  <tfoot className="bg-gray-50 font-black border-t">
                    <tr>
                      <td colSpan={4} className="px-4 py-2 text-right uppercase">Total dos Itens:</td>
                      <td className="px-4 py-2 text-right text-blue-700">R$ {itens.reduce((acc, i) => acc + i.subtotal, 0).toFixed(2)}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
